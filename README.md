...
- **Existence & location** are audited against **four independent authoritative sources** — [OpenStreetMap](https://www.openstreetmap.org) (Overpass, any-type spatial search within 2.5 km), [Wikidata](https://www.wikidata.org), the [US Census geocoder](https://geocoding.geo.census.gov), and the [Foursquare Places API](https://foursquare.com/products/places-api/) (`scripts/verify_sources.py` + `scripts/verify_foursquare.py`, applied by `migrations/007_multisource_verification.sql`). It is **precision-first**: a building is flagged `verified` only when a source confirms it **by name** near the pin. Guards reject town/region-name matches ("Long Island", "Five Towns"), road/street names, wrong house numbers, far crowd entries, and category mismatches — so a fabricated "Elmont Medical Center" can't ride on a neighbouring optometrist, and "Bayview Tower" can't ride on a water plant on Bayview Avenue. Of 572 buildings, **175** clear that bar (137 OSM, 5 Wikidata, 33 Foursquare). Address-only matches (the Census confirms the street address but no source confirms the building) do **not** verify — per-building evidence is in `scripts/sources_evidence.csv`.
- **Wrong coordinates** are corrected. The audit found **59** buildings whose seed pin was off and snapped them to the confirmed feature (only trustworthy name matches are moved).
- **Unverified buildings** carry an "Unverified" badge. Being unverified is **not** proof of fabrication — most have real street addresses but simply aren't named in any free dataset (only a commercial POI dataset like Google Places would have them) — so nothing is deleted.
- **Floor counts** are shown as a confirmed fact only where OpenStreetMap's `building:levels` backs them; every other floor count renders as an estimate (`· est`), even on otherwise-verified buildings (`migrations/005_honest_specs.sql`).
- **Elevator counts** have no authoritative public source anywhere, so they are **always** shown as a community estimate (`~N · est`) — never as a hard fact, on any building. Reviews can confirm real counts over time.
