...
- **Existence & location** are audited against **three independent authoritative sources** — [OpenStreetMap](https://www.openstreetmap.org) (Overpass, any-type spatial search within 2.5 km), [Wikidata](https://www.wikidata.org), and the [US Census geocoder](https://geocoding.geo.census.gov) (`scripts/verify_sources.py`, applied by `migrations/007_multisource_verification.sql`). It is **precision-first**: a building is flagged `verified` only when OSM or Wikidata confirms it **by name** near the pin. Guards reject town-name matches, road/street names, wrong house numbers, and category mismatches — so a fabricated "Elmont Medical Center" can't ride on a neighbouring optometrist, and "Bayview Tower" can't ride on a water plant on Bayview Avenue. Of 572 buildings, **141** clear that bar. Address-only matches (the Census confirms the street address but no source confirms the building) do **not** verify — per-building evidence is in `scripts/sources_evidence.csv`.
